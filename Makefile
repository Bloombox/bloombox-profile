...
all: build
	@echo "bloombox-profile is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-profile" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-profile"
endif
